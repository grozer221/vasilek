...
let Paginator: React.FC<PropsType> = ({ itemsCount, pageSize, currentPage, onPageChanged, portionSize = 5 }) => {
  ...
  return (
    <div className={s.wrapper_pagination}>
      { portionNumber > 1 &&
      <div className={s.pagination} onClick={() => { setPortionNumber(portionNumber - 1)}}><img src={arrowLeft}/></div>}
      {pages
      ...
      <div className={s.pagination} onClick={() => { setPortionNumber(portionNumber + 1)}}><img src={arrowRight}/></div>}
    </div>
  );
};
